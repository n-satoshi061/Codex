...
| 画面ID | 画面名 | 概要 |
| --- | --- | --- |
| SCR-01 | 在庫管理画面 | 在庫登録、在庫一覧表示、検索、絞り込み、数量更新、削除、買い物メモ表示を行う |
...
| F-03 | 在庫一覧表示 | 在庫を一覧表示する |
| F-04 | 在庫検索 | キーワードで在庫を絞り込む |
| F-05 | カテゴリ絞り込み | カテゴリ単位で在庫を絞り込む |
| F-06 | 数量更新 | ボタン操作で数量を増減する |
| F-07 | 在庫削除 | 不要在庫を削除する |
| F-08 | 買い物メモ表示 | 下限以下の在庫を買い物メモとして表示する |
